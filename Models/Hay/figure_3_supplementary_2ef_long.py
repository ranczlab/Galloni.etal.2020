...
h.load_file('simulationcode/critical_frequency_long.hoc')
...
plt.savefig('outputs/figures/traces/figure_3_supplementary_2e_long_%s.svg' % args.hotspot)
integral = np.trapz((vsoma[lower:upper] - vsoma[baseline])/10**6, tvec[lower:upper])

if path.exists('outputs/data/figure_3_supplementary_2f_long.csv'):
    with open(r'outputs/data/figure_3_supplementary_2f_long.csv', 'a') as f:
        writer = csv.writer(f)
        writer.writerow([integral])
else:
    with open(r'outputs/data/figure_3_supplementary_2f_long.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow([integral])