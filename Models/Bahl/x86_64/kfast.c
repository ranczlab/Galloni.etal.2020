...
#define nrn_init _nrn_init__kfast
#define _nrn_initial _nrn_initial__kfast
#define nrn_cur _nrn_cur__kfast
#define _nrn_current _nrn_current__kfast
#define nrn_jacob _nrn_jacob__kfast
#define nrn_state _nrn_state__kfast
#define _net_receive _net_receive__kfast 
#define _f_trates _f_trates__kfast 
#define rates rates__kfast 
#define states states__kfast 
#define trates trates__kfast 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_kfast", _hoc_setdata,
 "rates_kfast", _hoc_rates,
 "trates_kfast", _hoc_trates,
 0, 0
};
...
#define qa qa_kfast
 double qa = 9;
#define q10 q10_kfast
 double q10 = 2.3;
#define tha tha_kfast
 double tha = 25;
#define tadj tadj_kfast
 double tadj = 0;
#define temp temp_kfast
 double temp = 23;
#define usetable usetable_kfast
 double usetable = 1;
#define vmax vmax_kfast
 double vmax = 100;
#define vmin vmin_kfast
 double vmin = -120;
 ...
 "Ra_kfast", &Ra_kfast,
 "Rb_kfast", &Rb_kfast,
 "temp_kfast", &temp_kfast,
 "q10_kfast", &q10_kfast,
 "vmin_kfast", &vmin_kfast,
 "vmax_kfast", &vmax_kfast,
 "tadj_kfast", &tadj_kfast,
 "usetable_kfast", &usetable_kfast,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 kfast /Users/aeron/Github/shortneurons/Models/Bahl/x86_64/kfast.mod\n");
...
/*CVODE*/
 static int _ode_spec1 () {_reset=0;
 {
   ...
   Dn = ( ninf - n ) / ( timefactor_n * ntau ) ;
   }
 return _reset;
}
 static int _ode_matsol1 () {
 trates ( _threadargscomma_ v - vshift ) ;
 Dn = Dn  / (1. - dt*( ( ( ( - 1.0 ) ) ) / ( timefactor_n * ntau ) )) ;
  return 0;
}
 /*END CVODE*/
 ...
 {
   trates ( _threadargscomma_ v - vshift ) ;
    n = n + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / ( timefactor_n * ntau ))))*(- ( ( ( ninf ) ) / ( timefactor_n * ntau ) ) / ( ( ( ( - 1.0 ) ) ) / ( timefactor_n * ntau ) ) - n) ;
   }
  return 0;
}
 ...
 v=_v;
{
  ek = _ion_ek;
 { error =  states();
 if(error){fprintf(stderr,"at line 76 in file kfast.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/aeron/Github/shortneurons/Models/Bahl/channels/kfast.mod";
static const char* nmodl_file_text = 
  "\n"
  "COMMENT\n"
