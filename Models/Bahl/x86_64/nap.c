...
#define _f_settables _f_settables__nap 
#define settables settables__nap 
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 nap /Users/aeron/Github/shortneurons/Models/Bahl/x86_64/nap.mod\n");
...
static int _reset;
static char *modelname = "Sodium persistent current for RD Traub, J Neurophysiol 89:909-921, 2003";
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   settables ( _threadargscomma_ v - vshift ) ;
   Dm = ( minf - m ) / mtau ;
   }
 return _reset;
}
...
static int  _f_settables ( _threadargsprotocomma_ double _lv ) {
   minf = 1.0 / ( 1.0 + exp ( ( - _lv - 48.0 ) / 10.0 ) ) ;
   if ( _lv < - 40.0 ) {
     mtau = 0.025 + 0.14 * exp ( ( _lv + 40.0 ) / 10.0 ) ;
     }
   else {
     mtau = 0.02 + 0.145 * exp ( ( - _lv - 40.0 ) / 10.0 ) ;
     }
    return 0; }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/aeron/Github/shortneurons/Models/Bahl/channels/nap.mod";
static const char* nmodl_file_text = 
