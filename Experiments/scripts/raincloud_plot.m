...
%[h,u] = raincloud_plot('X',x,'box_on',1,'box_dodge',1,'bxfacecl',[1,1,1],'cloud_edge_col', [0.8 0.8 0.9],'point_size',30,'color',[0.7,0.9,1])
%
...
addOptional(p, 'point_size', 10, validScalarPosNum)
...
cloud_edge_col = p.Results.cloud_edge_col; band_width = p.Results.band_width; point_size = p.Results.point_size;
...
% density plot
h{1} = area(Xi, f, 'ShowBaseLine', 'off'); hold on
...
yl = get(gca, 'YLim');
set(gca, 'YLim', [-yl(2)*1.5*lwr_bnd yl(2)]);
...
% raindrops
if box_dodge
    drops_pos = (jit * 0.6) - yl(2) * 2*dot_dodge_amount;
else
    drops_pos = jit - yl(2) / 2;
end

h{2} = scatter(X, drops_pos);
h{2}.SizeData = point_size;
