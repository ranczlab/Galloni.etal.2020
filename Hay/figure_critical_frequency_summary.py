...
shortdata = []
with open('outputs/data/critical_frequency_short.csv') as csvfile:
    csvreader = csv.reader(csvfile, delimiter=',')
    for row in csvreader:
        shortdata.append(row[0])

longdata = []
with open('outputs/data/critical_frequency_long.csv') as csvfile:
    csvreader = csv.reader(csvfile, delimiter=',')
    for row in csvreader:
...
plt.xlim(40, 210)
plt.savefig('outputs/figures/figure_critical_frequency_summary.svg')