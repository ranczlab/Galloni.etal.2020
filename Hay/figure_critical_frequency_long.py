...
plt.savefig('outputs/figures/traces/critical_frequency_long_%s.svg' % args.hotspot)
integral = np.trapz((vsoma[lower:upper] - vsoma[baseline])/10**6, tvec[lower:upper])

if path.exists('outputs/data/critical_frequency_long.csv'):
    with open(r'outputs/data/critical_frequency_long.csv', 'a') as f:
        writer = csv.writer(f)
        writer.writerow([integral])
else:
    with open(r'outputs/data/critical_frequency_long.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow([integral])